...
class SimpleEnv:
    ...
    def reset(self, seed = None):
        ...
        q_zero = np.array([0.0, -0.785398, 0.0, -2.35619, 0.0, 1.57079, 0.785398], dtype=np.float32)
        ...
        self.q = np.concatenate([q_zero, np.array([0.04, 0.04])]) 
    ...
    def step(self, action):
        ...
        finger_pos = 0.04 * (1.0 - action[-1]) 
        gripper_cmd = np.array([finger_pos, finger_pos])
        self.compute_q = q
    ...
    def get_joint_state(self):
        ...
        gripper = self.env.get_qpos_joint('finger_joint1')
        gripper_cmd = 1.0 if gripper[0] < 0.01 else 0.0  # 闭合=1, 张开=0
    ...
    def get_delta_q(self):
        ...
        gripper = self.env.get_qpos_joint('finger_joint1')
        gripper_cmd = 1.0 if gripper[0] < 0.01 else 0.0 # 小于0.01m判定为闭合
    ...
    def check_success(self):
        ...
        if np.linalg.norm(p_mug[:2] - p_plate[:2]) < 0.1 and np.linalg.norm(p_mug[2] - p_plate[2]) < 0.6 and self.env.get_qpos_joint('finger_joint1') > 0.03:
